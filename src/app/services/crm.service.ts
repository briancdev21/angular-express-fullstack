...
export class CrmService {
  ...
  getLeadActivities(id): Observable<any> {
    const url = `${environment.apiUrl}crm/leads/${id}/recent-activities`;
    return this.http.get(url);
  }
  ...
  uploadLeadProfileImage (leadId, body): Observable<any> {
    const url = `${environment.apiUrl}crm/leads/${leadId}/lead-picture`;
  ...
  getContactActivities(id): Observable<any> {
    const url = `${environment.apiUrl}crm/contacts/${id}/recent-activities`;
    return this.http.get(url);
  }
  ...
  uploadContactProfileImage (leadId, body): Observable<any> {
    const url = `${environment.apiUrl}crm/contacts/${leadId}/contact-picture`;
    // const option = { headers: new HttpHeaders({ 'Content-Type': 'multipart/form-data' }) };
    return this.http.put(url, body)
      .map((res) => res);
  }
  ...
  convertLeadToContact(id) {
    const url = `${environment.apiUrl}crm/leads/${id}/contact-convert`;
